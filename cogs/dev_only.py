...
class dev_only(commands.Cog, name="Dev Only"):
    ...
    @discord.slash_command(name="dm", description="Send a message! also this is my first slash command", guild_ids=[main.DEVS_SERVER_ID])
    async def dm(self, interaction:Interaction, recipient: discord.Member, *, message: str):
    ...
    async def emoji(self, ctx, *, emojiname:str):
        ...
        for i in emojis:
            emojis_found.append(discord.utils.get(guild_emojis, name=i))
